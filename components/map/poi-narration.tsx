 ...
 * in a Google Maps-style bottom tab with smooth animations and avatar video support
 */
export default function PoiNarration() {
  ...
  // Show narration when activeNarration is set (from click or proximity)
  useEffect(() => {
    if (activeNarration) {
      setIsVisible(true);
      setIsExpanded(true);
      // Handle video for the active narration
      if (activeNarration.poi) {
        handleVideoForPOI(activeNarration.poi);
      }
    }
  }, [activeNarration]);
        ...
        >
          {activeNarration.isLoading ? (
            <div className="w-full p-8 flex flex-col items-center justify-center bg-gray-50">
              ...
              <p className="text-sm text-slate-600">Loading narration...</p>
            </div>
          ) : isLoadingVideo ? (
            <div className="w-full p-8 flex flex-col items-center justify-center bg-gray-50">
              <Loader2 className="w-12 h-12 text-blue-500 animate-spin mb-4" />
